...
def test_create_subject(db_session, client, user):
...
def test_get_subjects(db_session, client, user):
    response = login_user(client)
    assert response.status_code == 200
...
def test_create_subject_book(db_session, client, user, book_factory):
    response = login_user(client)
    assert response.status_code == 200
...
def test_subject_users(db_session, client, user, user_factory):
    response = login_user(client)
    assert response.status_code == 200
    ...
    response = create_subject_user(
        client, auth_token, subject_id, user_public_id=user_2.public_id
    )
    assert response.status_code == 200

    response = get_subject_user(
        client, auth_token, subject_id, user_public_id=user_1.public_id
    )
    assert response.status_code == 200
